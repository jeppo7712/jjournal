...
(the user's concrete case: IB01.L) originally logged as a stock trade
...
classified). Requested by an external API consumer (an automated
assistant tracking US-situs estate tax exposure for a non-US person: cash
at a US entity counts toward a combined threshold, cash elsewhere
doesn't) — the journal previously had no field saying where an account's
