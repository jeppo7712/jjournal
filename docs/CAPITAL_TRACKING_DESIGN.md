...
external API, not just trade PnL.
...
- New `GET /api/external/v1/holdings` — T-bills/bonds, filterable by
  ...
  actually earning" doesn't require an API consumer to cross-reference
  cash-transactions and do the yield math itself.

This gives an API consumer the full picture: total
...
after a futures account parent-linked to a stocks account but with zero
trades/cash of its own — correctly showed $0 everywhere, which looked
like a bug but wasn't). The concrete use case: one real brokerage account,
